                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  if(nrhs!=1) {
   mexPrintf("This function performs the Ewens exact and homozygosity\n");
   mexPrintf("tests as described in two papers in Genetical Research\n");   
   mexPrintf("by M. Slatkin (1994, 64: 71-74) and (1996, 68: 259-260).\n\n");
   mexErrMsgTxt("Usage: [pe,ph]=EWENSEXACT_MEX([9,2,1,1,1,1,1]);\nSpecify the configuration on the command line");
  } else if(nlhs>2) {
   mexErrMsgTxt("Usage: [pe,ph]=EWENSEXACT_MEX([9,2,1,1,1,1,1]);\nToo many output arguments");
	...
	if (r_tot >= RLIM)  {
		mexErrMsgTxt("n = %d is too large..\n", r_tot);
    	}
	if (k >= KLIMIT)  {
		mexErrMsgTxt("k = %d is too large.\n", k);
		}
	for (i=1;i<k; i++)
		if (r_obs[i] < r_obs[i+1])  {
			print_config(r_obs);
			mexErrMsgTxt(" is not a valid configuration.\n");
			}
	r_top = r_tot - 1;
	Fsig = Esig = 0;
	fill_factors();
	...
	print_config(r_obs);
	mexPrintf(":  P_E = %g, ", sig_sum / tot_sum);
	mexPrintf("P_H = %g\n", F_sig_sum / tot_sum);
    plhs[0] = mxCreateDoubleScalar(sig_sum / tot_sum);
    plhs[1] = mxCreateDoubleScalar(F_sig_sum / tot_sum);
  }  /*  end, mexFunction  */
